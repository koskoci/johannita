<template>
  <div id="page-edit">
    <v-form v-model="valid">
      <v-text-field
        v-model="page.attributes.short_name"
...
export default {
  ...
  data() {
    return {
      ...
      editor: new Editor({
        ...
        onUpdate: ({ getHTML }) => {
          this.html = getHTML();
          this.page.attributes.content = getHTML();
        },
      }),
      page: {
        id: 0,
  ...
  methods: {
    ...
    sendToServer() {
      ...
      const data = {
        type: 'page',
        attributes: {
          short_name: this.page.attributes.short_name,
          content: this.page.attributes.content,
        },
      };
      ...
      if (this.page.id === 0) {
        ...
        axios.patch(
          `${apiUrl}pages/${this.page.attributes.short_name}`,
          { data },
          {
            headers: {
              Authorization: `Bearer ${window.localStorage.getItem('user_token')}`,
  ...
  computed: {
    createdAt() {
      return new Date(this.page.attributes.created_at).toLocaleString('hu-HU');
    },
    updatedAt() {
      return new Date(this.page.attributes.updated_at).toLocaleString('hu-HU');
    },
  },
  mounted() {
    // console.log('mounted', this.$route.params);

    if (this.$route.name === 'admin-page-new') {
      console.log('new page');
    } else {
      axios
        .get(`http://206.189.55.142/api/pages/${this.$route.params.id}`)
        .then((response) => {
